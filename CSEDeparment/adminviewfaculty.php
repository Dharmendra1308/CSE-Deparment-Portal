...
if(isset($id))
{
    ...
    if($result)
    {
        header('location:adminviewfaculty.php');
    }
...
<head>
    ...
    <script type="text/javascript">
        ...
        function fun()
        {
            return confirm("Are you sure for remove this Faculty :");
        }
			...
			<ul class="navbar-nav">
				...
				<li class="nav-item dropdown">
					...
					<ul class="dropdown-menu" aria-labelledby='FacultyDropdown'>
						<li>
            				<a class="dropdown-item disabled" href="#">View Faculty</a>
            			</li>
            			<li>
...
<!-- Add Faculty -->
<div class="container">
    <div class="h2 text-success mb-3" style="text-align: center;">Total Faculty in CSE Deparment</div>
    
<?php
$link = mysqli_connect('localhost','root','','csedeptdb');
$qry = "select * from faculty where name != 'Admin'";
$result = mysqli_query($link,$qry);
...
if($n)
{
	...
	<table class='bg-secondary bg-opacity-25 shadow table table-bordered mb-4' align='center' cellpading='4px' >
            <tr align='center'><th>ID</th><th>Name</th><th>Mail</th><th>Mobile</th><th>Adhar</th><th>Post</th><th>Remove</th></tr>
TEST;
    
    while($r=mysqli_fetch_array($result))
    {
        $str .= "<tr align='center' style=\"font-family:'Footlight MT Light'; font-weight:normal;\" ><th> $r[id] </th><th> $r[name] </th><th> $r[mail] </th><th> $r[mobile] </th><th> $r[adhar] </th><th> $r[post] </th><th><a href='adminviewfaculty.php?id=$r[id]' class='btn text-danger' style='font-family:Constantia;' onclick='return fun()' >Remove</a></th></tr>";
    }
...
{
    echo "<div class='h3 text-danger' style='font-weight:bold; text-align:center;'>Faculty not Exist.</div>";
    mysqli_close($link);
}
