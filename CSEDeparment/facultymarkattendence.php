			...
			<ul class="navbar-nav">
				...
				<li class="nav-item dropdown">
					...
					<ul class="dropdown-menu" aria-labelledby='AttendenceDropdown'>
						<li>
            				<a class="dropdown-item disabled" href="#">Mark Attendence</a>
            			</li>
            			<li>
...
<div class="container-fluid">
	...
	<div class="row justify-content-center mb-3 h3 text-success">
		Today Attendence : <?php echo date('d-m-Y') ?>
	</div>

	<div class="container mb-5 p-3 justify-content-center">
...
{   
    ...
    $q = "select * from student where semester=$sem";
    ...
    if($n!=0)
    {
        $q1 = "select * from absentstudent where doc='".date('Y-m-d')."' and semester=$sem";
        $result1 = mysqli_query($link,$q1);
        $n1 = mysqli_num_rows($result1);
        
        $q2 = "select * from absenttable where doc='".date('Y-m-d')."' and semester=$sem";
        $result2 = mysqli_query($link,$q2);
        $n2 = mysqli_num_rows($result2);
        
        if(($n1 == null) && ($n2 == null))
        {
            echo "<center><b class='h3 text-danger'>CSE $sem Semester Student Attandance : ".date('d-F-Y')."</b></center><br />";
...
TEST;
            ...
            {
                $str .= "<tr><th> $r[rollno] </th><th> $r[name] </th><th><input type='checkbox' name='ch[]' value='$r[rollno]' /></th></tr>";
            }
        ...
        {
            mysqli_close($link);
            echo "<center class='text-success h3'>Attendence Already Marked.</center>";
        }
        
    }
    else
    {
        echo "<center class='h3 text-danger'>CSE $sem semester : No Record Found.</center>";
    }
}
else
    echo "<br><center class='text-danger h5'>Please Select Semester</center>";
...
{   
    ...
    mysqli_close($link);
    echo "<center class='text-success h3'>CSE $sem Semester : Attendence Successfully Marked. ".date('d-m-Y')."</center>";
}
...
if(isset($save) and !isset($ch))
{
    ...
    mysqli_close($link);
    echo "<center class='text-success h3'>CSE $sem Semester : Today All student are present. ".date('d-m-Y')."</center>";
}
